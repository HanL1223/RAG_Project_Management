...
def create_jira_text_splitter(
    ...
    jira_separators = [
        "\n\n---\n\n",  # Preferred section divider (Summary/Description/etc.)
    ...
    return RecursiveCharacterTextSplitter(
        ...
        lenght_function = len, # Use character count
...
def split_documents_with_ids(
    ...
    #iter each records in documents
    for doc in documents:
        issue_key = doc.metadata.get('issue_key','UNKNOWN')

        doc_chunk = splitter.split_documents([doc])

        for chunk_idx,chunk in enumerate(doc_chunk):
            #generate stable ID
            chunk_id = generate_chunk_id(
                issue_key= issue_key,
                chunk_index=chunk_idx,
                chunm_text = chunk.page_content,
            )
